...
def checkPin(checkInterval=2):
    global db_parameters, main_conf
    GPIO.wiringPiSetup()
    GPIO.pinMode(main_conf['powerPin'], 0) # Input mode
    GPIO.digitalWrite(main_conf['powerPin'], 0) # Starp power down
    while True:
        if main_conf['forceClose']:
            break
        if not GPIO.digitalRead(main_conf['powerPin']):
            db_parameters['isPowerOn'] = False
        else:
            db_parameters['isPowerOn'] = True
        updateDBParameters()
...
if __name__ == "__main__":
    ...
    dbThread.start()
    time.sleep(1)
    pinThread = threading.Thread(target=checkPin, args=(main_conf['dbCheckInterval'],))
    pinThread.start()
    time.sleep(1)
    while main_conf['is_db_updating']:
